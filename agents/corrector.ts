...
function buildCorrectorMessage(input: AgentInput): string {
    const parts: string[] = [];
...
export async function runCorrector(input: AgentInput): Promise<AgentOutput> {
    ...
    const model = routeModel(complexity, 'corrector');
    ...
    logAgentUsage('corrector', result.model, result.inputTokens, result.outputTokens, result.costUSD);
