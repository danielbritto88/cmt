...
exports.skillPrioritizeFailures = skillPrioritizeFailures;
exports.skillBuildCorrectionInstructions = skillBuildCorrectionInstructions;
exports.skillVerifyCorrection = skillVerifyCorrection;
exports.skillSummarizeCorrection = skillSummarizeCorrection;
// ─────────────────────────────────────────────
// SKILLS DO CORRETOR
// Funções auxiliares de correção cirúrgica
// ─────────────────────────────────────────────
// Isola as falhas mais críticas para corrigir primeiro
function skillPrioritizeFailures(failures) {
    const priority = {
...
function skillBuildCorrectionInstructions(report) {
    if (report.failures.length === 0)
        return '';
    const prioritized = skillPrioritizeFailures(report.failures);
    const lines = ['Corrija os seguintes problemas:'];
...
function skillVerifyCorrection(original, corrected, failures) {
    const resolved = [];
    const unresolved = [];
...
function skillSummarizeCorrection(resolved, unresolved) {
    const lines = [];
