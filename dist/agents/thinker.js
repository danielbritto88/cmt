...
// PARSING DO PLANO
// ─────────────────────────────────────────────
function parsePlan(raw, taskSummary) {
    try {
        ...
        const steps = Array.isArray(parsed.steps)
            ? parsed.steps.map((s, i) => ({
                ...
                agent: typeof s.agent === 'string' ? s.agent : 'executor',
            }))
            : [{ step: 1, description: taskSummary, agent: 'executor' }];
        return {
...
// AGENTE PENSADOR
// ─────────────────────────────────────────────
async function runThinker(input) {
...
`.trim();
    ...
    (0, logger_1.logAgentUsage)('thinker', result.model, result.inputTokens, result.outputTokens, result.costUSD);
