...
function getContextForTask(taskType) {
    switch (taskType) {
...
// ─────────────────────────────────────────────
function buildExecutorSystem(taskType) {
...
// ─────────────────────────────────────────────
function buildExecutorMessage(input) {
    const parts = [];
    if (input.briefing) {
...
async function runExecutor(input) {
    ...
    const model = (0, guardian_1.routeModel)(complexity, 'executor');
    const maxTokens = limits_1.TASK_LIMITS[taskType]?.maxTokens || 25000;
    ...
    (0, logger_1.logAgentUsage)('executor', result.model, result.inputTokens, result.outputTokens, result.costUSD);
    if (maestro_1.CMT_CONFIG.verbose) {
